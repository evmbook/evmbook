---
title: Cryptography
description: Keys, addresses, and the cryptographic foundations of Ethereum
chapter: 4
---
...
<Note>
Take a look at the following resources if you're interested in reading more about cryptography and the mathematical functions used in modern cryptography:

- [Cryptography (Wikipedia)](https://en.wikipedia.org/wiki/Cryptography)
- [Trapdoor function (Wikipedia)](https://en.wikipedia.org/wiki/Trapdoor_function)
- [Prime factorization (Wikipedia)](https://en.wikipedia.org/wiki/Integer_factorization)
...
More precisely, a private key can be any nonzero number up to a very large number slightly less than 2²⁵⁶—a huge 78-digit number, roughly 1.158 × 10⁷⁷. The exact number shares the first 38 digits with 2²⁵⁶ and is defined as the order of the elliptic curve used in Ethereum. To create a private key, we randomly pick a 256-bit number and check that it is within the valid range. In programming terms, this is usually achieved by feeding an even larger string of random bits (collected from a cryptographically secure source of randomness) into a 256-bit hash algorithm such as Keccak-256 or SHA-256, both of which will conveniently produce a 256-bit number. If the result is within the valid range, we have a suitable private key. Otherwise, we simply try again with another random number.

<Tip>
2²⁵⁶—the size of Ethereum's private key space—is an unfathomably large number. It is approximately 10⁷⁷ in decimal; that is, a number with 77 digits. For comparison, the visible universe is estimated to contain 10⁸⁰ atoms. Thus, there are almost enough private keys to give every atom in the universe an Ethereum account. If you pick a private key randomly, there is no conceivable way anyone will ever guess it or pick it themselves.
</Tip>
...
The *mod p* (modulo prime number *p*) indicates that this curve is over a finite field of prime order *p*, where *p* = 2²⁵⁶ – 2³² – 2⁹ – 2⁸ – 2⁷ – 2⁶ – 2⁴ – 1, which is a very large prime number.
...
You can verify this yourself using Python:
...
Elliptic curve addition is defined such that given two points *P₁* and *P₂* on the elliptic curve, there is a third point *P₃* = *P₁* + *P₂*, also on the elliptic curve.

Geometrically, this third point *P₃* is calculated by drawing a line between *P₁* and *P₂*. This line will intersect the elliptic curve in exactly one additional place (amazingly). Call this point *P₃'* = (*x*, *y*). Then reflect in the x-axis to get *P₃* = (*x*, *–y*).

In elliptic curve math, there is also a point called the "point at infinity," which roughly corresponds to the role of the number zero in addition. On computers, it's sometimes represented by *x* = *y* = 0 (which doesn't satisfy the elliptic curve equation, but it's an easy separate case that can be checked).
...
Let's apply this calculation to find the public key for the specific private key we showed you earlier:
...
In Ethereum you may see public keys represented as a serialization of 130 hexadecimal characters (65 bytes). This is adopted from a standard serialization format proposed by the industry consortium Standards for Efficient Cryptography Group (SECG). The standard defines four possible prefixes that can be used to identify points on an elliptic curve:
...
**OpenSSL** — The OpenSSL library offers a comprehensive set of cryptographic primitives, including a full implementation of `secp256k1`. For example, to derive the public key, the function `EC_POINT_mul` can be used.

**libsecp256k1** — Bitcoin Core's `libsecp256k1` is a C-language implementation of the `secp256k1` elliptic curve and other cryptographic primitives. It was written from scratch to replace OpenSSL in Bitcoin Core software, and is considered superior in both performance and security.

### Working with Keys in Practice

Modern development libraries abstract the cryptographic details. Here's how to work with keys using viem:
...
// 0x... (64 hex characters = 256 bits)

// Derive account (public key + address) from private key
const account = privateKeyToAccount(privateKey)

console.log('Address:', account.address)
// 0x... (40 hex characters = 160 bits = 20 bytes)

console.log('Public Key:', account.publicKey)
// 0x04... (130 hex characters = uncompressed public key)
```
...
// Signature contains r, s, and v components
console.log(signature)
// 0x... (132 hex characters = r + s + v)
```

<Callout type="info">
**Developer Note:** You should never generate or handle private keys in browser JavaScript unless absolutely necessary. For user-facing applications, let wallets like MetaMask manage keys. The examples above are for backend services, scripts, or learning purposes.
</Callout>

![Ethereum Cryptography Flow](/images/diagrams/ch04-cryptography-flow.svg)
...
**Indirect** — Encodes an identifier that resolves to an Ethereum address through a name registry provider. It uses 16 alphanumeric characters, comprising an *asset identifier* (e.g., ETH), a name service (e.g., XREG), and a 9-character human-readable name (e.g., KITTYCATS). Example: `XE##ETHXREGKITTYCATS` (20 characters long), where the `##` should be replaced by the two computed checksum characters.
...
The resulting mixed-capitals address:
...
## Modern Cryptographic Operations in Solidity

Solidity provides built-in cryptographic primitives that leverage the same mathematics covered in this chapter.
...
contract CryptoExamples {
    // Keccak-256 (Ethereum's primary hash function)
    function hashWithKeccak(string memory input) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(input));
    }

    // SHA-256 (Bitcoin's hash function, available in EVM)
    function hashWithSHA256(string memory input) public pure returns (bytes32) {
        return sha256(abi.encodePacked(input));
    }

    // RIPEMD-160 (used in Bitcoin addresses)
    function hashWithRIPEMD(string memory input) public pure returns (bytes20) {
...
Recovering the signer's address from a signature is a common pattern:
...
contract SignatureVerifier {
    ...
    // More commonly, use the full signature bytes
    function verify(
        bytes32 messageHash,
        ...
        // Adjust v for Ethereum's convention
        if (v < 27) v += 27;
...
**Security Note:** The `ecrecover` function returns `address(0)` for invalid signatures rather than reverting. Always check that the returned address is not zero and matches the expected signer.
</Callout>

### EIP-712: Typed Data Signing

For production applications, use EIP-712 structured data signing instead of raw message signing:
...
In this chapter we provided a survey of public key cryptography and focused on the use of public and private keys in Ethereum and the use of cryptographic tools, such as hash functions, in the creation and verification of Ethereum addresses. We also looked at digital signatures and how they can demonstrate ownership of a private key without revealing that private key.

### Key Takeaways for Developers

1. **Private keys are just numbers** — 256-bit random integers. Security depends entirely on randomness and secrecy.

2. **Public keys derive from private keys** — The elliptic curve math is one-way: easy to compute public from private, impossible to reverse.

3. **Addresses derive from public keys** — Keccak-256 hash, keep last 20 bytes. This is also one-way.

4. **Signatures prove ownership** — You can prove you control a private key without revealing it.

5. **Use libraries, don't implement** — Cryptographic code is subtle. Use battle-tested libraries like viem, ethers.js, or OpenZeppelin.

6. **EIP-55 checksums catch errors** — Always use mixed-case addresses when displaying to users.

7. **EIP-712 for signatures** — Typed data signing prevents users from signing malicious messages that look like transactions.
...
Public Key (512 bits = two 256-bit coordinates)
    ...
    ▼
Address (160 bits = 20 bytes = 40 hex chars)
```

Every step is one-way. You can go down but never back up.

In the next chapter on Accounts and Wallets, we will put these ideas together and look at how wallets manage collections of keys, derive addresses, and enable user interaction with the blockchain.
